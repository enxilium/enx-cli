...
# Migrate older fish wrapper generated by previous versions.
# Old line:
...
FISH_INIT_FILE="$HOME/.config/enx/shell/init.fish"
if [ -f "$FISH_INIT_FILE" ] && grep -q 'env ENX_FINALIZER_FILE="\$tmpfile" command enx \$argv' "$FISH_INIT_FILE"; then
    TMP_MIGRATE_FILE="${TMPDIR:-/tmp}/enx-fish-migrate.$$"
    awk '
        /env ENX_FINALIZER_FILE="\$tmpfile" command enx \$argv/ {
            print "    set -lx ENX_FINALIZER_FILE \"$tmpfile\""
            print "    command enx $argv"
            print "    set -e ENX_FINALIZER_FILE"
            next
        }
        { print }
    ' "$FISH_INIT_FILE" > "$TMP_MIGRATE_FILE"
    mv "$TMP_MIGRATE_FILE" "$FISH_INIT_FILE"
fi

echo ""
echo "install finished"
echo "if '$INSTALL_DIR' is not in PATH, add it to your shell profile"
echo "run 'enx setup' from the shell you want integrated (bash, zsh, or fish)"
